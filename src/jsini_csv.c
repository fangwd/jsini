 ...
 */
static int try_parse_csv_record(const char *buffer, size_t len, char delimiter, jsini_array_t **out_row) {
    ...
    while (p < end) {
        ...
        // Skip whitespace before quote? "Forgiving"?
        // Let's stick to: if it starts with quote, it's quoted.
        if (is_quote(*p)) {
            quote_char = *p++;
        }
...
int jsini_parse_file_csv_ex(const char *file, char delimiter, int has_header, jsini_jsonl_cb cb, void *user_data) {
    ...
    while (jsb_getline(&line_buf, fp) == JSB_OK) {
        ...
        int parse_res = try_parse_csv_record(sb.data, sb.size, delimiter, &row);
...
int jsini_print_file_csv(const char *file, const jsini_value_t *value, char delimiter) {
    if (value->type != JSINI_TARRAY) return JSINI_ERROR;
    ...
    if (first->type == JSINI_TOBJECT) {
        ...
        jsini_object_t *obj = (jsini_object_t*)first;
         ...
         for (i = 0; i < jsini_array_size(headers); i++) {
             if (i > 0) jsb_append_char(&sb, delimiter);
    ...
    for (i = 0; i < jsini_array_size(arr); i++) {
        ...
        if (headers && row_val->type == JSINI_TOBJECT) {
            ...
            for (k = 0; k < jsini_array_size(headers); k++) {
                if (k > 0) jsb_append_char(&sb, delimiter);
